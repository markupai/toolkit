...
const demoHandlers = {
  rewrite: {
    ...
    poll: http.get('*/v1/rewrites/:workflowId', () => {
      return HttpResponse.json(commonResponses.workflow.completed);
    }),
    failed: http.get('*/v1/rewrites/:workflowId', () => {
      return HttpResponse.json({
    ...
    emptyWorkflowId: http.post('*/v1/rewrites/', () => {
      return HttpResponse.json({ status: Status.Running });
    }),
  },
  check: {
    submit: http.post('*/v1/checks/', () => {
      return HttpResponse.json({
        status: Status.Running,
        workflow_id: 'test-workflow-id',
    ...
    poll: http.get('*/v1/checks/:workflowId', () => {
      return HttpResponse.json(commonResponses.workflow.completed);
    }),
    failed: http.get('*/v1/checks/:workflowId', () => {
      return HttpResponse.json({
        status: Status.Failed,
        workflow_id: 'test-workflow-id',
    ...
    emptyWorkflowId: http.post('*/v1/checks/', () => {
      return HttpResponse.json({ status: Status.Running });
    }),
  },
};
